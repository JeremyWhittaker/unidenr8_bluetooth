...
| ~~A cancelled state write could land *after* a later one~~ **— resolved** | `publish_state_async` hands the write to `asyncio.to_thread`, and cancelling that await — which the `wait_for` around a session does on every trial deadline and on shutdown — does not stop the worker thread. It finishes and renames whenever it is next scheduled, which can be after the shutdown publish already wrote `stopped`. A clean shutdown could therefore leave `state.json` reading `streaming` permanently, and the e-paper display would report a live detector link that nothing was holding. Two further races sat in the same function: a per-pid temporary filename shared by concurrent writers, and the process-global `os.umask` dance (leaking the *tighter* mask, so never a permissions loosening) | Every publish now takes a ticket when its document is **built**, and a write carrying an older ticket than the one already on disk is dropped rather than applied. The write body is serialised and the temporary carries the ticket. Found by a 3.12-only CI failure on a docs-only commit; the three protections are ablation-tested individually |
...
no publication of an address is involved in any of them. The publish-ordering
defect is the only one that could make the software *assert* something false to a
consumer, rather than merely fail to say something true.
